...
dataset_cache_dir=/local/a/pmaletti/datasets/
SEARCH_RESULTS_DIR="final_output/${model_name_or_path}/search_iter10_calib_samples16_ps30_ne10_mr0.2_cr0.7/"
...
base_hard_prune_path_template="final_output/${model_name_or_path}/ft/${BEST_KEY_CONFIG}/hard_pruned_models/hard_pruned_ckpt"

# Set output directory for CPT based on the selected config
cpt_output_dir="final_output/${model_name_or_path}/ft/${BEST_KEY_CONFIG}/CPT/"

echo "Using Strategy File: $EVOL_SEARCH_SHRINK_FILE"

# 4.3. GPU & BATCH CALCULATION
NUM_GPUS=$(nvidia-smi --query-gpu=name --format=csv,noheader | wc -l)
TARGET_GLOBAL_BS=128
PER_DEVICE_BS=4
...
if [[ -d "${cpt_ckpt_dir}" ]]; then
    ...
    # CUDA_LAUNCH_BLOCKING=1 torchrun --nproc_per_node=${NUM_GPUS} --master_port=${MASTER_PORT} 
    CUDA_LAUNCH_BLOCKING=1 CUDA_VISIBLE_DEVICES=2 python -m pdb main.py \
        --model_name_or_path "${model_name_or_path}" \
