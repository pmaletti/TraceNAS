...
hard_pruned_dir="${scratch_dir}/final_output/${model_name_or_path}/ft/${BEST_KEY_CONFIG}/CPT/hard_pruned_ckpt"
output_dir="${scratch_dir}/final_output/${model_name_or_path}/ft/${BEST_KEY_CONFIG}/CPT"
...
if find "${output_dir}" -maxdepth 1 -type d -name "checkpoint-*" | grep -q .; then
    ...
    for task in "${lm_eval_tasks[@]}"; do
        ...
        CUDA_LAUNCH_BLOCKING=0 python main.py \
