...
from transformers.tracenas_utils.scoring_funcs import *
from transformers.tracenas_utils.CustomWSDScheduler import *
...
class Trainer:
    ...
    def _get_dataloader(
        ...
        dataloader_params = {
            ...
            "shuffle": False if self.args.get_influence else None
        }
    ...
    def create_scheduler(self, num_training_steps: int, optimizer: torch.optim.Optimizer = None):
        ...
        if self.args.lr_scheduler_type == 'CustomWSDScheduler':
            kwargs = self.args.lr_scheduler_kwargs
            self.lr_scheduler = CustomWSDScheduler(
    ...
    def save_population_checkpoint(
        ...
        population_data,
        base_paths,
        selection_mode='repeated',
        layer_pruning=False
    ):
        """
        Filters, sorts, and saves the population state for a given iteration.
        
        Args:
            search_iter (int): Current iteration number.
            population_data (dict): Dictionary containing lists for active_layers, width_masks, 
                                    scores, structures, params, losses, latency, and throughput.
            base_paths (dict): Dictionary containing base file paths for scores, layers, masks, etc.
            selection_mode (str): 'repeated', 'pareto', or 'diversity'.
            layer_pruning (bool): Whether layer pruning is enabled (affects mask saving).
        """
        
        # 1. Generate periodic filenames
        def get_periodic_path(path):
            base, ext = os.path.splitext(path)
            return f"{base}_iter_{search_iter}{ext}"

        periodic_paths = {k: get_periodic_path(v) for k, v in base_paths.items()}
        
        print(f"\nSaving checkpoint at iteration {search_iter}...")

        # 2. Extract population lists for easier access
        p_layers = population_data['active_layers']
        p_masks = population_data['width_masks']
        p_scores = population_data['influence_scores']
        p_structures = population_data['structures']
        p_params = population_data['params']
        p_losses = population_data['losses']
        p_sparsity = population_data['sparsity']
        p_latency = population_data['latency']
        p_throughput = population_data['throughput']

        # 3. Filter structures based on selection mode
        if selection_mode == 'repeated':
            # Assumes get_repeated_values_lwr is defined globally or imported
            selected_data = get_repeated_values_lwr(p_structures, p_scores)
        elif selection_mode == 'pareto':
            selected_data = get_pareto_optimal_structures(
                p_structures, p_scores, p_losses, p_latency, p_throughput
            )
        elif selection_mode == 'diversity':
            selected_data = get_diversity_preserving_pareto(
                p_structures, p_scores, p_latency, p_throughput
            )

        # 4. Rebuild population from selected indices
        best_indices = [data['index'] for data in selected_data.values()]
        
        # Sort by influence score and take top 100
        sorted_idx = sorted(best_indices, key=lambda i: p_scores[i], reverse=True)[:100]

        # 5. Extract final sorted subsets
        final_layers = [p_layers[i] for i in sorted_idx]
        final_masks = [p_masks[i] for i in sorted_idx]
        final_scores = [p_scores[i] for i in sorted_idx]
        final_structures = [p_structures[i] for i in sorted_idx]
        final_params = [p_params[i] for i in sorted_idx]
        final_losses = [p_losses[i] for i in sorted_idx]
        final_sparsity = [p_sparsity[i] for i in sorted_idx]
        final_latency = [p_latency[i] for i in sorted_idx]
        final_throughput = [p_throughput[i] for i in sorted_idx]

        # 6. Save data to files
        with open(periodic_paths['influence_scores'], "w") as fid:
            for i in range(len(final_scores)):
                fid.write(
                    f"{final_scores[i]}-----{final_params[i]}-----{final_structures[i]}-----"
                    f"{final_losses[i]}-----{final_latency[i]}-----{final_throughput[i]}\n"
                )

        np.save(periodic_paths['active_layers'], final_layers)        
        
        if not layer_pruning:                        
            np.save(periodic_paths['active_width_mask'], final_masks)
    ...
    def training_step(
        ...
        """
        if not self.args.get_influence:
            ...
            if not self.args.eval_after_evolsearch:
                # Constants for evolutionary algorithm
                population_size = 30 # Total individuals in the population
                num_elites = 10 #20 #10    # Number of top individuals to carry over directly
                num_guaranteed_elites = max(1, int(num_elites * 0.5)) # e.g., guarantee 50% of the elites are the very best
                num_random_elites = num_elites - num_guaranteed_elites
                mutation_rate_active_layers = 0.2 #0.2
                mutation_rate_width_mask_entry = 0.2 #0.2 Probability to flip a single mask bit
                crossover_rate = 0.7

                output_dir_path = os.path.join(
                        self.args.output_dir, f'search_iter{self.args.max_num_iters}_calib_samples{self.args.max_train_samples}_ps{population_size}_ne{num_elites}_mr{mutation_rate_active_layers}_cr{crossover_rate}'
                    )
                os.makedirs(output_dir_path, exist_ok=True)

                influence_scores_path = os.path.join(
                    output_dir_path, f'influence_scores_seed{self.args.seed}.txt'
                )
                
                active_layer_path = os.path.join(
                    output_dir_path, f'active_layers_seed{self.args.seed}.npy'
                )

                active_width_mask_path = os.path.join(
                    output_dir_path, f'active_width_mask_seed{self.args.seed}.npy'
                )

                layer_importance_attn_path = os.path.join(
                    output_dir_path, f'layer_importance_attn{self.args.seed}.npy'
                )

                layer_importance_mlp_path = os.path.join(
                    output_dir_path, f'layer_importance_mlp{self.args.seed}.npy'
                )
                bias_path = os.path.join(
                    output_dir_path, f'bias_seed{self.args.seed}.pt'
                )

                base_paths = {
                    "influence_scores": influence_scores_path,
                    "active_layers": active_layer_path,
                    "active_width_mask": active_width_mask_path,
                    "layer_importance_attn": layer_importance_attn_path,
                    "layer_importance_mlp": layer_importance_mlp_path,
                    "bias": bias_path
                }

                if os.path.isfile(influence_scores_path) or os.path.isfile(active_layer_path) or os.path.isfile(active_width_mask_path) or os.path.isfile(bias_path):
                    print(f"File '{influence_scores_path}' or '{active_layer_path}' or '{active_width_mask_path}' or '{bias_path}' already exists. Skipping writing.")
                    exit()


                # from attacc_simulator.main import main as attac_sim

                # ### Step 1 compute base_grad_vec for tracin and iHVP influence calculation
                start_time = time.time()
                total_base_loss = 0.0
                accumulated_base_grad_vec = None
                num_batches_for_base = 0
                if self.args.shrinking_method == 'calib_dp':
                    self.strategy = np.load(self.args.shrinking_file, allow_pickle=True).item()

                if hasattr(self, 'mmlu_dataset'):
                    data_loader = self.get_eval_dataloader(self.mmlu_dataset)
                elif hasattr(self, 'wikitext2_dataset'):
                    data_loader = self.get_eval_dataloader(self.wikitext2_dataset)
                else:
                    data_loader = self.get_train_dataloader()
                
                fixed_batches = [val for val in iter(data_loader)]

                base_grad_vecs_attn, base_grad_vecs_mlp, base_grad_attn_fisher, base_grad_mlp_fisher, batch_base_loss, input_tensors_attn, input_tensors_mlp, base_grad_vecs_attn_o, base_grad_vecs_mlp_down = calculate_base_influence(self.args, model, fixed_batches, self.compute_loss_context_manager, self.compute_loss, self.accelerator)

                save_base_scores(input_tensors_attn, input_tensors_mlp, f'{output_dir_path}/base_inputs_seed{self.args.seed}/')
                # save_base_scores(base_grad_vecs_attn_o, base_grad_vecs_mlp_down, f'{output_dir_path}/base_grads_seed{self.args.seed}/')

                if '70B' not in self.args.model_name_or_path or 'qwen' not in self.args.model_name_or_path.lower():
                    layer_scores = get_layer_importance(model, input_tensors_attn, input_tensors_mlp)
                else:
                    print('here!')
                    ...
                    layer_scores = {
                        'attn': {i: 1.0 for i in range(num_layers)}, 
                        'mlp': {i: 1.0 for i in range(num_layers)}
                    }

                # exit()
                model_config = model.config.text_config if hasattr(model.config, 'text_config') else model.config
                if not self.args.shrinkable_width:
                    layers_var_info = None
                with self.compute_loss_context_manager():                    
                    if self.args.enable_shrinking:
                        population_active_layers = []
                        population_width_masks =[]
                        population_influence_scores = []
                        ...
                        for search_iter in range(self.args.max_num_iters):
                            per_iter_start_time = time.time()
                            if search_iter>0 and (search_iter==1 or search_iter % 2 == 0):
                                population_data = {
                                    'active_layers' : population_active_layers,
                                    'width_masks' : population_width_masks,
                                    'influence_scores' : population_influence_scores,
                                    'structures' : population_structures,
                                    'params' : population_params,
                                    'losses' : population_losses,
                                    'sparsity' : population_sparsity,
                                    'latency' : population_latency,
                                    'throughput' : population_throughput,
                                }
                                ...
                                self.save_population_checkpoint(
                                    ...
                                    population_data,
                                    base_paths,
                                    layer_pruning = self.args.layer_pruning
                                )

                                del population_data
                                # assert [(int(i[0].sum().item()), int(i[1].sum().item())) for i in population_active_layers] == [(int(l_a), int(l_m)) for (l_a, l_m, w) in population_structures]
                                
                                # base_filename, file_extension = os.path.splitext(influence_scores_path)
                                # periodic_scores_path = f"{base_filename}_iter_{search_iter}{file_extension}"
                                
                                # base_layers_filename, layers_extension = os.path.splitext(active_layer_path)
                                # periodic_layers_path = f"{base_layers_filename}_iter_{search_iter}{layers_extension}"

                                # base_masks_filename, masks_extension = os.path.splitext(active_width_mask_path)
                                # periodic_masks_path = f"{base_masks_filename}_iter_{search_iter}{masks_extension}"

                                # base_attn_importance_filename, extension = os.path.splitext(layer_importance_attn_path)
                                # periodic_attn_importance_path = f"{base_attn_importance_filename}_iter_{search_iter}{extension}"

                                # base_mlp_importance_filename, extension = os.path.splitext(layer_importance_mlp_path)
                                # periodic_mlp_importance_path = f"{base_mlp_importance_filename}_iter_{search_iter}{extension}"

                                # # base_bias_filename, extension = os.path.splitext(bias_path)
                                # # periodic_bias_path = f"{base_bias_filename}_iter_{search_iter}{extension}"

                                # print(f"\nSaving checkpoint at iteration {search_iter}...")
                                
                                # repeated_structures = get_repeated_values_lwr(population_structures, population_influence_scores)

                                # #Scores that dominate only
                                # # repeated_structures = get_pareto_optimal_structures(
                                # #                             population_structures, 
                                # #                             population_influence_scores, 
                                # #                             population_losses,
                                # #                             population_latency,
                                ...
                                # #Keeps the diversity
                                # # repeated_structures = get_diversity_preserving_pareto(
                                # #                             population_structures, 
                                # #                             population_influence_scores, 
                                # #                             population_latency,
                                # #                             population_throughput
                                ...
                                # best_population_active_layers = []
                                # best_population_width_masks = []
                                # best_population_influence_scores = []
                                # best_population_structures = []
                                # best_population_params = []

                                # best_population_losses = []
                                # best_population_sparsity = []
                                # best_population_latency = []
                                # best_population_throughput = []

                                # for lw, data in repeated_structures.items():
                                #     best_index = data['index']
                                #     best_population_active_layers.append(population_active_layers[best_index])
                                #     best_population_width_masks.append(population_width_masks[best_index])
                                #     # best_population_width_masks.append(copy.deepcopy(data['mask']))
                                #     best_population_influence_scores.append(population_influence_scores[best_index])
                                #     best_population_structures.append(population_structures[best_index])
                                #     best_population_params.append(population_params[best_index])

                                #     best_population_losses.append(population_losses[best_index])
                                #     best_population_sparsity.append(population_sparsity[best_index])
                                #     best_population_latency.append(population_latency[best_index])
                                #     best_population_throughput.append(population_throughput[best_index])

                                # sorted_idx = sorted(range(len(best_population_influence_scores)), key=lambda sub: best_population_influence_scores[sub], reverse=True)[:100]
                                # best_population_active_layers = [best_population_active_layers[idx] for idx in sorted_idx]
                                # best_population_width_masks = [best_population_width_masks[idx] for idx in sorted_idx]
                                # best_population_influence_scores = [best_population_influence_scores[idx] for idx in sorted_idx]
                                # best_population_structures = [best_population_structures[idx] for idx in sorted_idx]
                                # best_population_params = [best_population_params[idx] for idx in sorted_idx]

                                # best_population_losses = [best_population_losses[idx] for idx in sorted_idx]
                                # best_population_sparsity = [best_population_sparsity[idx] for idx in sorted_idx]
                                # best_population_latency = [best_population_latency[idx] for idx in sorted_idx]
                                # best_population_throughput = [best_population_throughput[idx] for idx in sorted_idx]

                                # # Save influence scores
                                # with open(periodic_scores_path, "w") as fid:
                                #     # fid.write(f"(l,w) combination provided is just for reference. Actual values can be found at {periodic_layers_path} and {periodic_masks_path}\n")
                                #     for i in range(len(best_population_influence_scores)):
                                #         score = best_population_influence_scores[i]
                                #         structure = best_population_structures[i]
                                #         params = best_population_params[i]
                                #         loss = best_population_losses[i]
                                #         latency = best_population_latency[i]
                                #         throughput = best_population_throughput[i]
                                #         fid.write(f"{score}-----{params}-----{structure}-----{loss}-----{latency}-----{throughput}\n")

                                # np.save(periodic_layers_path, best_population_active_layers)        
                                # if not self.args.layer_pruning:                        
                                #     # torch.save(best_population_width_masks, periodic_masks_path)
                                #     np.save(periodic_masks_path, best_population_width_masks)
                            ...
                            if search_iter==0:
                                ...
                                # while len(population_active_layers) < 2: 
                                min_layer, max_layer = self.args.min_num_layer, model_config.num_hidden_layers
                                if self.args.layer_pruning:
                                    width_options = [eval(self.args.width_choice)]
                                else:
                                    width_options = list(eval(self.args.width_choice))

                                #Get masks for the base model once 
                                full_active_idx=torch.where(torch.ones(max_layer))[0]
                                full_active_idx= full_active_idx.tolist()

                                attn_layer_sizes, mlp_layer_sizes = get_layer_sizes(model, full_active_idx, full_active_idx)
                                layer_sizes = {'attn': attn_layer_sizes, 'mlp': mlp_layer_sizes}
                                print(attn_layer_sizes)
                                fisher_mask_paths = {}
                                fisher_masks = {}
                                sparsity_dict = {}
                                ...
                                if not self.args.layer_pruning:
                                    ...
                                    for w in width_options:
                                        layer_tracin_attn = {layer_idx: 0 for layer_idx in full_active_idx}
                                        layer_tracin_mlp = {layer_idx: 0 for layer_idx in full_active_idx}

                                        target_width = w # This is the fraction to KEEP
                                        fisher_mask, sparsity = compute_pruning_masks(
                                                                        self.args, 
                                                                        model, 
                                                                        target_width, 
                                                                        layer_scores, 
                                        ...
                                        saved_filename = create_sparsity_buffer(sparsity, target_width, buffer_dir=f'{output_dir_path}/tmp/sparsity_cache/gen_{search_iter}')

                                        fisher_mask_paths[w] = saved_filename
                                        fisher_masks[w] = fisher_mask
                                        sparsity_dict[w] = sparsity
                                else:
                                    fisher_mask_paths = {eval(self.args.width_choice): None}
                                    fisher_masks = {eval(self.args.width_choice): None}
                                    sparsity_dict = {eval(self.args.width_choice):{'attn': None, 'mlp': None}}

                                model_config = model.config.text_config if hasattr(model.config, 'text_config') else model.config
                                while len(population_active_layers) < 2:
                                    num_combinations_init = num_elites #population_size // 2
                                    chosen_combinations = set() # Use a set to automatically handle uniqueness

                                    for w in width_options:
                                        chosen_combinations.add((max_layer, max_layer, w))

                                    # Keep generating combinations until we have enough unique ones
                                    if not self.args.layer_pruning:
                                        nci= num_combinations_init
                                    else:
                                        nci= num_combinations_init
                                    while len(chosen_combinations) < nci:
                                        if self.args.min_num_layer == model_config.num_hidden_layers:
                                            eval_num_layer_attn = model_config.num_hidden_layers
                                            eval_num_layer_mlp = model_config.num_hidden_layers
                                        else:
                                            eval_num_layer_attn = np.random.randint(min_layer, max_layer + 1)
                                            # eval_num_layer_mlp = np.random.randint(min_layer, max_layer + 1)
                                            eval_num_layer_mlp = eval_num_layer_attn
                                        eval_num_width = np.random.choice(width_options).item()
                                        chosen_combinations.add((eval_num_layer_attn, eval_num_layer_mlp, eval_num_width))
                                    # sparsity_dict = {}
                                    for p_i, (l_attn, l_mlp, w) in enumerate(chosen_combinations):   
                                        active_layers_attn = torch.ones(max_layer)
                                        active_layers_mlp = torch.ones(max_layer)
                                        num_inactive_needed = max_layer - l_attn

                                        # 1. Generate random indices from 0 to (max_layer - 2)
                                        #    This explicitly excludes the last index (max_layer - 1)
                                        available_indices = max_layer - 1
                                        inactive_idx = torch.randperm(available_indices)[:num_inactive_needed]
                                        # 2. Set those layers to inactive
                                        active_layers_attn[inactive_idx] = 0
                                        assert active_layers_attn.sum().item()==l_attn

                                        #Generate random combination for MLP
                                        active_layers_mlp = active_layers_attn.clone()

                                        assert active_layers_mlp.sum().item()==l_mlp
                                        ...
                                        fisher_mask_path = fisher_mask_paths.get(w)
                                        fisher_mask = fisher_masks.get(w)

                                        influence_score, fisher_mask, rand_loss, layer_tracin_attn, layer_tracin_mlp, avg_latency, throughput = calculate_influence_w_l2normdist(self.args, model, fixed_batches, active_layers_attn, active_layers_mlp, width_choice, base_grad_vecs_attn, base_grad_vecs_mlp, base_grad_attn_fisher, base_grad_mlp_fisher, batch_base_loss, self.compute_loss_context_manager, self.compute_loss, self.accelerator, fisher_mask=fisher_mask, base_attn_tensors=input_tensors_attn, base_mlp_tensors=input_tensors_mlp)

                                        if self.args.number_of_params_thresh is not None and influence_score is None:
                                            # print(f'Structure: {(active_layers_attn.sum().item(), active_layers_mlp.sum().item() ,w)}, Params: {model_size} ,Loss: {rand_loss}, \n')
                                            # exit()
                                            continue
                                        
                                        if torch.isnan(influence_score):
                                            # print(f'Structure: {(active_layers_attn.sum().item(), active_layers_mlp.sum().item() ,w)}, Params: {model_size} ,Loss: {rand_loss}, \n')
                                            # exit()
                                            continue
                                        model_size = count_active_parameters(model, active_layers_attn, active_layers_mlp, fisher_mask)[1]

                                        population_active_layers.append((active_layers_attn, active_layers_mlp))
                                        population_width_masks.append(fisher_mask_path)
                                        population_influence_scores.append(influence_score.cpu().item()) #/active_layers_attn.sum().item())
                                        population_losses.append(rand_loss)
                                        population_sparsity.append(sparsity_dict[w])
                                        population_latency.append(avg_latency)
                                        population_throughput.append(throughput)

                                        if self.args.layer_pruning:
                                            population_structures.append((l_attn, l_mlp, 1))
                                            population_params.append(model_size)
                                        else:
                                            w = find_closest_width(fisher_mask, list(eval(self.args.width_choice)))
                                            population_structures.append((l_attn,l_mlp, w))
                                            population_params.append(model_size)


                                        print(f'Structure: {(active_layers_attn.sum().item(), active_layers_mlp.sum().item() ,w)}, Params: {model_size} ,Loss: {rand_loss}, Influence Score: {influence_score.cpu().item()}, \n')

                                sorted_idx = sorted(range(len(population_influence_scores)), key=lambda sub: population_influence_scores[sub], reverse=True)
                                population_active_layers = [population_active_layers[idx] for idx in sorted_idx]
                                population_width_masks = [population_width_masks[idx] for idx in sorted_idx]
                                population_influence_scores = [population_influence_scores[idx] for idx in sorted_idx]
                                population_structures = [population_structures[idx] for idx in sorted_idx]
                                population_params = [population_params[idx] for idx in sorted_idx]

                                population_losses = [population_losses[idx] for idx in sorted_idx]
                                population_sparsity = [population_sparsity[idx] for idx in sorted_idx]
                                population_latency = [population_latency[idx] for idx in sorted_idx]
                                population_throughput = [population_throughput[idx] for idx in sorted_idx]

                            else:
                                # exit()
                                # print(f"\n--- Evolution Search Iteration {search_iter} ---")
                                model_config = model.config.text_config if hasattr(model.config, 'text_config') else model.config

                                new_population_active_layers = []
                                new_population_width_masks = []
                                new_population_influence_scores = []
                                new_population_structures = []
                                new_population_params = []
                                new_chosen_combinations = set()
                                new_population_losses = []
                                new_population_sparsity = []
                                new_population_latency = []
                                new_population_throughput = []

                                # 1. Elitism: Carry over top individuals
                                if len(population_active_layers) > num_guaranteed_elites:
                                    for i in range(num_guaranteed_elites):
                                        new_population_active_layers.append(population_active_layers[i])
                                        new_population_width_masks.append(population_width_masks[i])
                                        new_population_influence_scores.append(population_influence_scores[i])
                                        new_population_structures.append(population_structures[i])
                                        new_population_params.append(population_params[i])

                                        new_population_losses.append(population_losses[i])
                                        new_population_sparsity.append(population_sparsity[i])
                                        new_population_latency.append(population_latency[i])
                                        new_population_throughput.append(population_throughput[i])


                                else:
                                    new_population_active_layers = population_active_layers
                                    new_population_width_masks = population_width_masks
                                    new_population_influence_scores = population_influence_scores
                                    new_population_structures = population_structures
                                    new_population_params = population_params

                                    new_population_losses = population_losses
                                    new_population_sparsity = population_sparsity
                                    new_population_latency = population_latency
                                    new_population_throughput = population_throughput
                                ...
                                if self.args.min_num_layer == model_config.num_hidden_layers:
                                    total_combinations = len(range(min_layer,max_layer+1)) * len(width_options)
                                    if total_combinations < num_guaranteed_elites + num_random_elites:
                                        num_new_combinations = total_combinations
                                    else:
                                        num_new_combinations = num_guaranteed_elites + num_random_elites
                                else:
                                    num_new_combinations = num_guaranteed_elites + num_random_elites


                                for new_struct in new_population_structures:
                                    new_chosen_combinations.add(new_struct) # Use a set to automatically handle uniqueness

                                # Keep generating combinations until we have enough unique ones
                                while len(new_chosen_combinations) < num_new_combinations:
                                    if self.args.min_num_layer == model_config.num_hidden_layers:
                                        eval_num_layer_attn = model_config.num_hidden_layers
                                        eval_num_layer_mlp = model_config.num_hidden_layers
                                    else:
                                    ...
                                    eval_num_width = np.random.choice(width_options)
                                    # eval_num_width = 1
                                    new_chosen_combinations.add((eval_num_layer_attn, eval_num_layer_mlp, eval_num_width))

                                new_chosen_combinations = list(new_chosen_combinations)
                                layers_var_info=None
                                # Generate the rest of the population
                                # full_width_structures = {}
                                p_i=0
                                while len(new_population_active_layers) < population_size:
                                    #Choose parents for evolution search
                                    parent1, parent1_width_mask, \
                                    parent2, parent2_width_mask = generate_parents(
                                                                                self.args, new_population_active_layers, new_population_width_masks,
                                                                                new_chosen_combinations, self.strategy, model_config.num_hidden_layers, fixed_batches, layers_var_info
                                                                            )
                                    
                                    granularity, choices =get_granularity(self.args, model_config.num_hidden_layers, random.randint(1,10))

                                    if self.args.layer_pruning:
                                        offspring_active_layers_attn, offspring_width_mask = generate_offsprings(self.args, width_options, 
                                                                                model, fixed_batches,
                                                                                parent1[0], parent1_width_mask, 
                                                                                parent2[0], parent2_width_mask,
                                                                                granularity, choices, layers_var_info, crossover_rate, mutation_rate_width_mask_entry, mutation_rate_active_layers
                                                                                )
                                        offspring_active_layers_mlp = offspring_active_layers_attn
                                        # 2. Type conversion (Standardize to Tensor)
                                        if isinstance(offspring_active_layers_attn, np.ndarray):
                                            offspring_active_layers_attn = torch.from_numpy(offspring_active_layers_attn)
                                        elif isinstance(offspring_active_layers_attn, list):
                                            offspring_active_layers_attn = torch.tensor(offspring_active_layers_attn)

                                        if isinstance(offspring_active_layers_mlp, np.ndarray):
                                            offspring_active_layers_mlp = torch.from_numpy(offspring_active_layers_mlp)
                                        elif isinstance(offspring_active_layers_mlp, list):
                                            offspring_active_layers_mlp = torch.tensor(offspring_active_layers_mlp)

                                        # 1. Calculate target sum (l_attn)
                                        l_attn = offspring_active_layers_attn.sum().item()
                                        l_mlp = offspring_active_layers_mlp.sum().item()
                                    else:
                                        offspring_active_layers_attn, offspring_width_mask = generate_offsprings(self.args, width_options, 
                                                                                model, fixed_batches,
                                                                                parent1[0], parent1_width_mask, 
                                    ...
                                    active_layers_attn = offspring_active_layers_attn
                                    active_layers_mlp = offspring_active_layers_mlp
                                    if self.args.layer_pruning:
                                        w = 1
                                    else:
                                        width_choice = np.random.choice(list(eval(self.args.width_choice)))
                                        w = width_choice
                                    ...
                                    # Compute masks for each layer combination...
                                    active_idx_attn = torch.where(active_layers_attn)[0]
                                    active_idx_mlp = torch.where(active_layers_attn)[0]

                                    attn_layer_sizes, mlp_layer_sizes = get_layer_sizes(model, active_idx_attn, active_idx_mlp)
                                    layer_sizes = {'attn': attn_layer_sizes, 'mlp': mlp_layer_sizes}
                                    
                                    if not self.args.layer_pruning:
                                        target_width = width_choice # This is the fraction to KEEP

                                        best_sparsity = next(
                                            (val for (attn, mlp, width), val in zip(new_population_structures, new_population_sparsity) if width == target_width), 
                                            None
                                        )

                                        if best_sparsity is None:
                                            best_sparsity=[]
                                        else:
                                            best_sparsity=[best_sparsity]

                                        all_matching_sparsities = [
                                            val for (attn, mlp, width), val in zip(new_population_structures, new_population_sparsity) 
                                            if width == w
                                        ]

                                        if not all_matching_sparsities:
                                            best_sparsity = []
                                        else:
                                            best_sparsity = all_matching_sparsities

                                        # -----------------------------------------------------------------------------------------
                                        offspring_width_mask, sparsity = compute_pruning_masks(
                                                                            self.args, 
                                                                            model, 
                                                                            target_width, 
                                                                            layer_scores, 
                                    ...
                                    offspring_influence_score, fisher_mask, rand_loss, layer_tracin_attn, layer_tracin_mlp, avg_latency, throughput = calculate_influence_w_l2normdist(self.args, model, fixed_batches, active_layers_attn, active_layers_mlp, width_choice, base_grad_vecs_attn, base_grad_vecs_mlp, base_grad_attn_fisher, base_grad_mlp_fisher, batch_base_loss, self.compute_loss_context_manager, self.compute_loss, self.accelerator, fisher_mask=offspring_width_mask, base_attn_tensors=input_tensors_attn, base_mlp_tensors=input_tensors_mlp)
                                    
                                    if self.args.number_of_params_thresh is not None and offspring_influence_score is None:
                                        continue
                                    if torch.isnan(offspring_influence_score):
                                        continue
                                    model_size = count_active_parameters(model, active_layers_attn, active_layers_mlp, fisher_mask)[1]
                                    
                                    if not self.args.layer_pruning:
                                        struct_string = "".join(map(str, active_layers_attn.int().tolist()))
                                        struct_hash = hashlib.md5(struct_string.encode()).hexdigest()[:8]
                                        saved_filename = create_sparsity_buffer(sparsity, target_width, buffer_dir=f'{output_dir_path}/tmp/sparsity_cache/gen_{search_iter}/pop_{p_i}_{struct_hash}')
                                        offspring_mask_path = saved_filename
                                    else:
                                        offspring_mask_path = None

                                    new_population_active_layers.append((offspring_active_layers_attn, offspring_active_layers_mlp)) 
                                    new_population_width_masks.append(offspring_mask_path)
                                    new_population_influence_scores.append(offspring_influence_score.cpu().item()) 
                                    new_population_losses.append(rand_loss)
                                    new_population_sparsity.append(sparsity)
                                    new_population_latency.append(avg_latency)
                                    new_population_throughput.append(throughput)
                                    
                                    w = find_closest_width(fisher_mask, list(eval(self.args.width_choice)))

                                    print(f'Structure: {(int(offspring_active_layers_attn.sum().item()), int(offspring_active_layers_mlp.sum().item()), w)}, Params: {model_size}, Loss: {rand_loss}, Influence Score: {offspring_influence_score.cpu().item()}, \n')

                                    
                                    if self.args.layer_pruning:
                                        new_population_structures.append((l_attn, l_mlp, 1))
                                        population_params.append(model_size)
                                    else:
                                        new_population_structures.append((int(offspring_active_layers_attn.sum().item()), int(offspring_active_layers_mlp.sum().item()), w))
                                        new_population_params.append(model_size)                                    
                                    p_i+=1

                                # Update the population for the next iteration
                                population_active_layers += new_population_active_layers
                                population_width_masks += new_population_width_masks
                                population_influence_scores += new_population_influence_scores
                                population_structures += new_population_structures
                                population_params += new_population_params

                                population_losses += new_population_losses
                                population_sparsity += new_population_sparsity
                                population_latency += new_population_latency
                                population_throughput += new_population_throughput
                            ...
                            per_iter_end_time = time.time()
                            print(f'Iter: {search_iter}, time: {per_iter_end_time-per_iter_start_time}')
                ...
                # for structure in set(repeated_structures):
                #     best_score = -1
                #     best_index = -1

                #     # 3. Iterate through the entire population to find all instances of the current structure.
                #     for i, s in enumerate(population_structures):
                #         if s == structure:
                #             current_score = population_influence_scores[i]

                #             # 4. Check if the current instance has a better influence score.
                #             if current_score > best_score:
                #                 best_score = current_score
                #                 best_index = i

                #     # 5. After checking all instances, append the data of the best one to our new lists.
                #     if best_index != -1:
                #         best_population_structures.append(population_structures[best_index])
                #         best_population_influence_scores.append(population_influence_scores[best_index])
                #         best_population_active_layers.append(population_active_layers[best_index])
                ...
                # population_active_layers = best_population_active_layers
                # population_width_masks = best_population_width_masks
                # population_influence_scores = best_population_influence_scores
                # population_structures = best_population_structures
                
                # with open(influence_scores_path, "w") as fid:
                #     for i, idx in enumerate(sorted_idx):
                #         temp = str(population_influence_scores[idx]) + '-----' + str(population_structures[idx])
                    
                #         fid.write(temp)
                #         fid.write("\n")
                # np.save(active_layer_path, population_active_layers)
                # torch.save(population_width_masks, active_width_mask_path)

            else:
                def get_file_with_largest_iter(folder_path, pattern):
                    ...
                    iter_regex = re.compile(r'_iter_(\d+)\.(\w+)$')
                    
                    def get_iter_number(filename):
                        match = iter_regex.search(filename)
                        return int(match.group(1)) if match else -1
                    
                    # Get the file with the highest iteration
                    file_with_largest_iter = max(all_files, key=get_iter_number)
                    
                    # IMPORTANT: If the highest found iter is -1, it means no files matched the regex
                    if get_iter_number(file_with_largest_iter) == -1:
                        return None
                        
                    return file_with_largest_iter
                ...
                output_dir_path = self.args.output_dir

                active_layer_path = os.path.join(
                    output_dir_path, f'active_layers_seed42.npy'
                )

                active_width_mask_path = os.path.join(
                    output_dir_path, f'active_width_mask_seed42.pt'
                )

                influence_scores_path = os.path.join(
                    output_dir_path, f'influence_scores_seed{self.args.seed}_{self.args.influence_type}_noratio_final_es{self.args.max_train_samples}_bs5_rank{self.args.lora_r}_cl{self.args.target_max_len}.txt'
                )
                
                structures_file = get_file_with_largest_iter(output_dir_path, pattern="influence_scores_seed42_iter_*.txt") 

                with open(structures_file, 'r') as f:
                    population_structures_raw = f.read().split('\n')

                    population_structures_ = [line for line in population_structures_raw if line.strip()]

                initial_influence_scores = []
                population_structures = []
                for line in population_structures_:
                    temp_split = line.split('-----')
                    score = temp_split[0]
                    l_w_tuple = tuple(eval(temp_split[2]))
                    initial_influence_scores.append(score)
                    population_structures.append(l_w_tuple)
            
                del population_structures_
                del population_structures_raw

                active_layers_file = get_file_with_largest_iter(output_dir_path, pattern="active_layers_seed42_iter_*.npy")
                population_active_layers = np.load(active_layers_file, allow_pickle=True)

                # 1. Try finding .pt iteration first
                active_width_mask_file = get_file_with_largest_iter(
                    output_dir_path, 
                    pattern=f"active_width_mask_seed42_iter_*.pt"
                )

                # 2. Try finding .npy iteration if .pt failed
                if active_width_mask_file is None:
                    active_width_mask_file = get_file_with_largest_iter(
                        output_dir_path, 
                        pattern=f"active_width_mask_seed42_iter_*.npy"
                    )

                # --- LOADING LOGIC ---
                if active_width_mask_file and os.path.exists(active_width_mask_file):
                    print(f"Loading mask from: {active_width_mask_file}")
                    
                    if active_width_mask_file.endswith('.pt'):
                        population_width_masks = torch.load(active_width_mask_file, weights_only=False)
                    else:
                        # Defaults to .npy loading
                        population_width_masks = np.load(active_width_mask_file, allow_pickle=True)
                    # print(population_width_masks)
                else:
                    print("Warning: No mask file found at any priority level.")
                    print(active_width_mask_file)
                    exit()
                ...
                fixed_batches = [val for val in iter(data_loader)]
                print(f'Number of batches for influence calculation: {len(fixed_batches)} \n')
                print(fixed_batches)
                if self.args.influence_type in ['tracin', 'tracin_w_log_act','tracin_w_l2normdist', 'tracin_w_zico', 'tracin_w_cosine', 'tracin_w_dot_prod']:
                    base_grad_vecs_attn, base_grad_vecs_mlp, base_grad_attn_fisher, base_grad_mlp_fisher, batch_base_loss, input_tensors_attn, input_tensors_mlp, output_tensors_attn, output_tensors_mlp = calculate_base_influence(self.args, model, fixed_batches, self.compute_loss_context_manager, self.compute_loss, self.accelerator)
                ...
                for i, (l_a, l_m, target_width) in enumerate(population_structures):
                    active_layers_attn, active_layers_mlp = population_active_layers[i]
                    width_choice = target_width
                    
                    if self.args.influence_type=='tracin_w_l2normdist':
                        if 'npy' in active_width_mask_file:
                            path = population_width_masks[i]
                            with open(path, 'r') as f:
                                sparsity_config = json.load(f)
                            
                            sparsity_config = {
                                outer_key: {int(inner_key): value for inner_key, value in inner_dict.items()}
                                for outer_key, inner_dict in sparsity_config.items()
                            }

                            full_active_idx = np.where(active_layers_attn)[0]
                            attn_layer_sizes, mlp_layer_sizes = get_layer_sizes(model, full_active_idx, full_active_idx)
                            layer_sizes = {'attn': attn_layer_sizes, 'mlp': mlp_layer_sizes}

                            fisher_mask, sparsity = compute_pruning_masks(self.args, model, target_width, None, attn_layer_sizes, mlp_layer_sizes, input_tensors_attn, input_tensors_mlp, best_sparsity=sparsity_config)
                            
                            influence_score, fisher_mask, rand_loss, layer_tracin_attn, layer_tracin_mlp, avg_latency, throughput = calculate_influence_w_l2normdist(self.args, model, fixed_batches, active_layers_attn, active_layers_mlp, width_choice, base_grad_vecs_attn, base_grad_vecs_mlp, base_grad_attn_fisher, base_grad_mlp_fisher, batch_base_loss, self.compute_loss_context_manager, self.compute_loss, self.accelerator, fisher_mask=fisher_mask, base_attn_tensors=input_tensors_attn, base_mlp_tensors=input_tensors_mlp, base_attn_op_tensors=output_tensors_attn, base_mlp_op_tensors=output_tensors_mlp)
                        else:
                            influence_score, fisher_mask, rand_loss, layer_tracin_attn, layer_tracin_mlp, avg_latency, throughput = calculate_influence_w_l2normdist(self.args, model, fixed_batches, active_layers_attn, active_layers_mlp, width_choice, base_grad_vecs_attn, base_grad_vecs_mlp, base_grad_attn_fisher, base_grad_mlp_fisher, batch_base_loss, self.compute_loss_context_manager, self.compute_loss, self.accelerator, fisher_mask=population_width_masks[i], base_attn_tensors=input_tensors_attn, base_mlp_tensors=input_tensors_mlp, base_attn_op_tensors=output_tensors_attn, base_mlp_op_tensors=output_tensors_mlp)
                    
                    elif self.args.influence_type=='tracin_w_cosine':
                        if 'npy' in active_width_mask_file:
                            path = population_width_masks[i]
                            with open(path, 'r') as f:
                                sparsity_config = json.load(f)
                            ...
                            influence_score, fisher_mask, rand_loss, layer_tracin_attn, layer_tracin_mlp, avg_latency, throughput = calculate_influence_w_cosine(self.args, model, fixed_batches, active_layers_attn, active_layers_mlp, width_choice, base_grad_vecs_attn, base_grad_vecs_mlp, base_grad_attn_fisher, base_grad_mlp_fisher, batch_base_loss, self.compute_loss_context_manager, self.compute_loss, self.accelerator, fisher_mask=fisher_mask, base_attn_tensors=input_tensors_attn, base_mlp_tensors=input_tensors_mlp, base_attn_op_tensors=output_tensors_attn, base_mlp_op_tensors=output_tensors_mlp)
                        else:
                            influence_score, fisher_mask, rand_loss, layer_tracin_attn, layer_tracin_mlp, avg_latency, throughput = calculate_influence_w_cosine(self.args, model, fixed_batches, active_layers_attn, active_layers_mlp, width_choice, base_grad_vecs_attn, base_grad_vecs_mlp, base_grad_attn_fisher, base_grad_mlp_fisher, batch_base_loss, self.compute_loss_context_manager, self.compute_loss, self.accelerator, fisher_mask=population_width_masks[i], base_attn_tensors=input_tensors_attn, base_mlp_tensors=input_tensors_mlp, base_attn_op_tensors=output_tensors_attn, base_mlp_op_tensors=output_tensors_mlp)
                    
                    elif self.args.influence_type=='tracin_w_dot_prod':
                        if 'npy' in active_width_mask_file:
                            path = population_width_masks[i]
                            with open(path, 'r') as f:
                                sparsity_config = json.load(f)
                            ...
                            influence_score, fisher_mask, rand_loss, layer_tracin_attn, layer_tracin_mlp, avg_latency, throughput = calculate_influence_w_dot_prod(self.args, model, fixed_batches, active_layers_attn, active_layers_mlp, width_choice, base_grad_vecs_attn, base_grad_vecs_mlp, base_grad_attn_fisher, base_grad_mlp_fisher, batch_base_loss, self.compute_loss_context_manager, self.compute_loss, self.accelerator, fisher_mask=fisher_mask, base_attn_tensors=input_tensors_attn, base_mlp_tensors=input_tensors_mlp, base_attn_op_tensors=output_tensors_attn, base_mlp_op_tensors=output_tensors_mlp)
                        else:
                            influence_score, fisher_mask, rand_loss, layer_tracin_attn, layer_tracin_mlp, avg_latency, throughput = calculate_influence_w_dot_prod(self.args, model, fixed_batches, active_layers_attn, active_layers_mlp, width_choice, base_grad_vecs_attn, base_grad_vecs_mlp, base_grad_attn_fisher, base_grad_mlp_fisher, batch_base_loss, self.compute_loss_context_manager, self.compute_loss, self.accelerator, fisher_mask=population_width_masks[i], base_attn_tensors=input_tensors_attn, base_mlp_tensors=input_tensors_mlp, base_attn_op_tensors=output_tensors_attn, base_mlp_op_tensors=output_tensors_mlp)

                    elif self.args.influence_type=='zico':
                        influence_score, fisher_mask, rand_loss = compute_zico(self.args, model, fixed_batches, active_layers_attn, active_layers_mlp, width_choice, self.compute_loss_context_manager, self.compute_loss, self.accelerator, population_width_masks[i])

                    elif self.args.influence_type=='meco':
                        influence_score, fisher_mask, rand_loss = compute_meco(self.args, model, fixed_batches, active_layers_attn, active_layers_mlp, width_choice, self.compute_loss_context_manager, self.compute_loss, self.accelerator, population_width_masks[i])

                    elif self.args.influence_type=='naswot':
                        influence_score, fisher_mask, rand_loss = compute_naswot(self.args, model, fixed_batches, active_layers_attn, active_layers_mlp, width_choice, self.compute_loss_context_manager, self.compute_loss, self.accelerator, population_width_masks[i])

                    elif self.args.influence_type=='gradnorm':
                        influence_score, fisher_mask, rand_loss = compute_gradnorm(self.args, model, fixed_batches, active_layers_attn, active_layers_mlp, width_choice, self.compute_loss_context_manager, self.compute_loss, self.accelerator, population_width_masks[i])

                    elif self.args.influence_type=='pruner_zero':
                        influence_score, fisher_mask, rand_loss = compute_pruner_zero(self.args, model, fixed_batches, active_layers_attn, active_layers_mlp, width_choice, self.compute_loss_context_manager, self.compute_loss, self.accelerator, population_width_masks[i])
                    
                    elif self.args.influence_type=='lpzero':
                        influence_score, fisher_mask, rand_loss = compute_lpzero(self.args, model, fixed_batches, active_layers_attn, active_layers_mlp, width_choice, self.compute_loss_context_manager, self.compute_loss, self.accelerator, population_width_masks[i])
                    
                    elif self.args.influence_type in ['synaptic_saliency', 'synaptic_diversity']:
                        influence_score, fisher_mask, rand_loss = compute_synaptic_(self.args, model, fixed_batches, active_layers_attn, active_layers_mlp, width_choice, self.compute_loss_context_manager, self.compute_loss, self.accelerator, population_width_masks[i])
                    
                    elif self.args.influence_type=='snip':
                        influence_score, fisher_mask, rand_loss = compute_SNIP(self.args, model, fixed_batches, active_layers_attn, active_layers_mlp, width_choice, self.compute_loss_context_manager, self.compute_loss, self.accelerator, population_width_masks[i])

                    elif self.args.influence_type in ['synflow', 'logsynflow']:
                        influence_score, fisher_mask = compute_synflow(self.args, model, fixed_batches, active_layers_attn, active_layers_mlp, width_choice, self.compute_loss_context_manager, self.compute_loss, self.accelerator)

                    elif self.args.influence_type == 'fisher':
                        influence_score, fisher_mask = compute_fisher(self.args, model, fixed_batches, active_layers_attn, active_layers_mlp, width_choice, self.compute_loss_context_manager, self.compute_loss, self.accelerator)
                    
                    print(f'{i}, Structure: {(l_a, l_m, target_width)}, {self.args.influence_type} score: {influence_score}')
                    population_influence_scores.append(influence_score)
                    population_params.append(count_active_parameters(model, active_layers_attn, active_layers_mlp, fisher_mask)[1])
                    population_loss.append(rand_loss)
                ...
                sorted_idx = sorted(range(len(population_influence_scores)), key=lambda sub: population_influence_scores[sub], reverse=True)[:100]
                ...
                # Save influence scores
                with open(influence_scores_path, "w") as fid:
                    for i in range(len(population_influence_scores)):
                        score = population_influence_scores[i]
                        structure = population_structures[i]
                        params = population_params[i]
                        loss=population_loss[i]
                        fid.write(f"{score}-----{params}-----{structure}-----{loss}\n")
                print(f'saved to: {influence_scores_path}')

                np.save(active_layer_path, population_active_layers)        
                # if not self.args.layer_pruning:                        
                #    torch.save(population_width_masks, active_width_mask_path)

                base_attn_importance_filename = os.path.join(
                    output_dir_path, f'layer_importance_attn{self.args.seed}_new.npy'
                )
                base_mlp_importance_filename = os.path.join(
                    output_dir_path, f'layer_importance_mlp{self.args.seed}_new.npy'
                )
    ...
    def compute_loss(
        ...
        return (loss, outputs) if return_outputs else ((loss, total_time_s) if return_runtime else loss)
