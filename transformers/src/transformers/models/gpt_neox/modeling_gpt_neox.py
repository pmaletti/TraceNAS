...
class GPTNeoXMLP(nn.Module):
    def __init__(self, config, layer_idx=None):
        ...
        # 1. Determine Dynamic Intermediate Size
        if hasattr(config, "intermediate_size_list") and layer_idx is not None:
        ...
        # 2. Define Layers with Pruned Dimensions
        # Pythia: hidden_size -> pruned_intermediate_size
        self.dense_h_to_4h = nn.Linear(
            config.hidden_size, 
            self.intermediate_size, 
            bias=getattr(config, "mlp_bias", True)
        )
        
        # Pythia: pruned_intermediate_size -> hidden_size
...
class GPTNeoXAttention(nn.Module):
    def __init__(self, config, layer_idx=None):
        ...
        # 1. Determine Pruned Dimensions
        if hasattr(config, "num_attention_heads_list") and hasattr(config, "hidden_size_list"):
            # Use specific pruned head count for this layer
            self.num_heads = config.num_attention_heads_list[layer_idx]
            # Pythia/NeoX usually keeps a constant head_size even when pruning heads
            self.head_size = config.hidden_size // config.num_attention_heads
            
            # Pruned query/key/value dimension
            qkv_dim = self.num_heads * self.head_size
        else:
            # Fallback to defaults
            self.num_heads = config.num_attention_heads
            self.head_size = config.hidden_size // config.num_attention_heads
            qkv_dim = config.hidden_size
        ...
        # In MHA, Q, K, and V all share the same pruned dimension
        ...
        # 4. Output Projection
        # Needs to map from pruned dimension back to original hidden size
...
class GPTNeoXLayer(GradientCheckpointingLayer):
    def __init__(self, config, layer_idx):
        ...
        if is_mlp_dead:
            ...
            self.mlp = GPTNeoXMLP(config, layer_idx=layer_idx)
...
class GPTNeoXForCausalLM(GPTNeoXPreTrainedModel, GenerationMixin):
    ...
    def __init__(self, config, **kwargs):
        ...
        self.gpt_neox = GPTNeoXModel(config, **kwargs)
        # self.model = GPTNeoXModel(config, **kwargs)
    ...
    def set_active_layers(self, active_layers_attn, active_layers_mlp, width=None):  # [num_layer] with boolean values
        ...
        self.gpt_neox.set_active_layers(active_layers_attn, active_layers_mlp, width=width)

    def get_input_embeddings(self):
        # GPTNeoXModel uses .embed_in for its input embeddings
