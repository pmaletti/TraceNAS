...
class LoraModel(BaseTuner):
    ...
    **Attributes**:
    ...
    def __init__(self, model, config, adapter_name) -> None:
        ...
        # self.use_moe_lora_coeff = config[adapter_name].use_moe_lora_coeff

        if hasattr(config, "use_moe_lora"):
            self.use_moe_lora = config.use_moe_lora
        elif isinstance(config, dict) and adapter_name in config:
            self.use_moe_lora = config[adapter_name].use_moe_lora
        else:
            self.use_moe_lora = False # Default fallback

        if hasattr(config, "use_moe_lora_coeff"):
            self.use_moe_lora_coeff = config.use_moe_lora_coeff
        elif isinstance(config, dict) and adapter_name in config:
            self.use_moe_lora_coeff = config[adapter_name].use_moe_lora_coeff
        else:
            self.use_moe_lora_coeff = False # Default fallback

        
        if hasattr(config, "kaiming_init"):
            self.kaiming_init = config.kaiming_init
        else:
            self.kaiming_init = False

        model_config = self.model.config.text_config if hasattr(self.model.config, 'text_config') else self.model.config 
        if self.use_moe_lora or self.use_moe_lora_coeff:
            from .moe import SparseMoE, SoftMoE
            
            assert config[adapter_name].width_choice is not None
            self.width_choice = eval(config[adapter_name].width_choice)
            
            if isinstance(self.width_choice, int):
                self.width_choice = [self.width_choice]
                print('sparse_moe',self.width_choice)
            self.lora_sparsemoe = SparseMoE(
                n_embed=model_config.num_hidden_layers + len(self.width_choice),
                num_experts=config[adapter_name].num_experts,
                top_k=config[adapter_name].top_k,
            )        
            # self.lora_sparsemoe = SparseMoE(
            #     n_embed=self.model.config.num_hidden_layers + 1,
            #     num_experts=config[adapter_name].num_experts,
            #     top_k=config[adapter_name].top_k,
            # )        

            # self.lora_sparsemoe = SoftMoE(
    ...
    def set_active_layers(self, active_layers_attn, active_layers_mlp, width=None):  # [num_layer] with boolean values
        if type(active_layers_attn) is not torch.Tensor:
            if isinstance(active_layers_attn, list):
                active_layers_attn = np.array(active_layers_attn)
        ...
        if type(active_layers_mlp) is not torch.Tensor:
            if isinstance(active_layers_mlp, list):
                active_layers_mlp = np.array(active_layers_attn)
        ...
        if hasattr(self.model.model, 'language_model'):
            self.model.model.language_model.set_active_layers(active_layers_attn, active_layers_mlp)
        else:
            self.model.set_active_layers(active_layers_attn, active_layers_mlp)
            
        if (hasattr(self, 'use_moe_lora') and self.use_moe_lora) or (hasattr(self, 'use_moe_lora_coeff') and self.use_moe_lora_coeff):
            assert width is not None
            if isinstance(width, np.str_):
            ...
            width_feature[idx] = 1
            # width_feature = torch.tensor([width])
            # print(active_layers_mlp)
            # print(width_feature)
            moe_feature = torch.cat([active_layers_mlp, width_feature]) #.to(next(self.model.parameters()))
            ...
            for module in self.model.modules():
                ...
                if hasattr(module, 'set_moe_gate'):
                    # module.set_moe_gate(gating_scores.to(module.weight.device), hard_indices.to(module.weight.device))  ## assume active_layers_mlp == active_layers_attn
                    module.set_moe_gate(gating_scores.to(module.weight.device).detach(), hard_indices.to(module.weight.device).detach())  ## assume active_layers_mlp == active_layers_attn
    ...
    def _create_and_replace(
        ...
        kwargs = {
            ...
            "use_moe_lora_coeff": lora_config.use_moe_lora_coeff,
            "n_embed": model_config.num_hidden_layers,
        ...
        if isinstance(target, LoraLayer) and not isinstance(target, AdaLoraLayer):
            target.update_layer(
                ...
                use_rslora=lora_config.use_rslora,
                use_dora=lora_config.use_dora,
                kaiming_init=lora_config.kaiming_init
            )
        else:
            new_module = self._create_new_module(lora_config, adapter_name, target, kaiming_init=lora_config.kaiming_init, **kwargs)
    ...
    @staticmethod
    def _create_new_module(lora_config, adapter_name, target, kaiming_init=None, **kwargs):
        ...
        for dispatcher in dispatchers:
            new_module = dispatcher(target, adapter_name, lora_config=lora_config, kaiming_init=kaiming_init, **kwargs)
